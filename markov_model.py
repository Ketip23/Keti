...
class MarkovModel(object):
    ...
    def kgram_freq(self, kgram):
        if self._k != len(kgram):
            ...
            return 0
        count = 0
        for v in self._st[kgram].values():
            count += v
        return count
    ...
    def rand(self, kgram):
        ...
        chars_list = []
        for letter in self._st[kgram]:
            for i in range(self._st[kgram][letter]):
                chars_list.append(letter)

        stdrandom.shuffle(chars_list)
        index = stdrandom.uniformInt(0, len(chars_list))
...
# Given a list a, _argmax returns the index of the maximum value in a.
    ...
    def replace_unknown(self, corrupted):
        ...
        for i in range(len(corrupted)):
            if corrupted[i] == '~':
                kgram = corrupted[i - self._k:i]
                c_list = self._st[kgram].keys()
                change = [kgram[1:] + c for c in c_list]
                for v in c_list:
                    probs_list = []
                    probs = 1
                    for k in range(1, self._k + 1):
                        numerator = self.char_freq(kgram, v)
                        denominator = float(self.kgram_freq(kgram))
                        probs *= numerator / denominator
                        probs_list.append(probs)
                index = _argmax(probs_list)
